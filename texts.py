...
begin_tapped = 'Приступаем, ищу Ваши данные'
...
help_message = """Если что-то пошло не так или хотите изменить свой номер/имя, пишите @bot_dealla
...
enter_narrative = """Выберите сюжет. Если нужный сюжет отсутствует, можете написать текстом"""
...
Требуется 2 фото. Каждое фото/документ до 20 МБ.
...
Требуется 4 фото. Каждое фото/документ до 20 МБ.

<i>Вы можете прислать больше фото в одном сообщении, они будут приняты, а вы перекинуты далее по боту.</i>"""
error_photo = 'Это не похоже на фото'
go_to_work = '<b>Фото принято, приступайте к работе.</b>'
...
photo_need_reached = '<b>Нужное количество фото достигнуто</b>'

enter_representative = "Напишите юр.лицо перевозчика"

go_to_demontage = '<b>Фото принято, приступайте к демонтажу</b>'
